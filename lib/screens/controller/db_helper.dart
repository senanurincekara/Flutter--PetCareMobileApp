    ...
    try {
      ...
      _database = await openDatabase(
        ...
        onCreate: (db, version) {
          ...
          return db.execute(
            'CREATE TABLE $_tableName (id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, note TEXT, date STRING, startTime STRING, endTime STRING, remind INTEGER, repeat STRING, isCompleted INTEGER, completedAt STRING, createdAt STRING, updatedAt STRING)',
          );
        },
      );
    } catch (e) {
  ...
  static Future<int> updateTaskInfo(Task task) async {
    ...
    try {
      return await _database!.rawUpdate(
        '''
      UPDATE $_tableName
      SET title = ?, note = ?, date = ?, startTime = ?, endTime = ?, remind = ?, repeat = ?, isCompleted = ?, createdAt = ?, updatedAt = ?, completedAt = ?
      WHERE id = ?
      ''',
        [
          task.title,
